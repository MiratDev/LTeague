$ ->
  definitions = [
    ...
    {
      word: 'usability testing' # Alt version of the above
      definition: 'A person who uses or interacts with the web application. The user is always part of one or more stakeholder groups.'
    },
    {
  ...
  for key, val of definitions
    $(".service-step-body, .services .sub-head").html (_, html) ->
      if ($(this).text().toUpperCase().indexOf(val['word'].toUpperCase()) >= 0) or ($(this).text().toUpperCase().indexOf((val['alt'] || '').toUpperCase()) >= 0)
        # re = new RegExp("((\\b)(#{val['word']}|#{val['alt']})(\\b|$))","i");
        re = new RegExp("(?!<.*?)((\\b)(#{val['word']}|#{val['alt']})(\\b|$))(?![^<>]*?(</span>|>))","i");
        html.replace(re, "<span class=\"definition\" data-tooltip=\"#{val["definition"]}\">$1</span>")
